...
internal static class Router
{
    ...
    private const string ProtocolVersion = "2026-04-06-v1";
    ...
    public static async Task HandleAsync(HttpListenerContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (request.HttpMethod.Equals("GET", StringComparison.OrdinalIgnoreCase) &&
                request.Url?.AbsolutePath == "/debug/nodes")
            {
                var debugNodes = await GameThread.InvokeAsync(GameStateService.BuildDebugNodesPayload);
                await WriteJsonAsync(response, 200, new
                {
                    ok = true,
                    request_id = requestId,
